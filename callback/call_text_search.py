...
def call_text_search(settings:dict, data:dict, instance:dict):
    ...
    if status == 0:
        ...
        # 실패명 로그만 남기고 진행
        if status != 0:
            myutils.log_message(f'[call_text_search][error]==>insert_doc:{res}\n')
        ...
        myutils.log_message(f'[call_text_search][error]==>call_callback:{error}=>{response}\n')
    ...
    myutils.log_message(f"\t[call_text_search][end]=>time:{el_time}")
    myutils.log_message(f"-" * 50)
