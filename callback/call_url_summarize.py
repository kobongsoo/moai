...
sys.path.append('..')
from utils import generate_text_GPT2, generate_text_davinci, generate_Gemma, generate_Gemini_single

def call_url_summarize(settings:dict, data:dict, instance:dict):
    ...
    myutils.log_message(f"\t[call_url_summarize][start]=>max_token:{max_tokens},temperature:{temperature},top_p:{top_p},stream:{stream}")
    ...
    if status == 0:
        ...
        myutils.log_message(f'[call_url_summarize][error]==>call_callback:{error}=>{response}\n')
    ...
    template = callback_template.template_url_summarize(query=query, response=response, elapsed_time=el_time)              

    # id_manager 에 id 제거 :응답 처리중에는 다른 질문할수 없도록 lock 기능을 위한 user_id 제거
    id_manager.remove_id_all(user_id) # id 제거
    
    myutils.log_message(f"\t[call_url_summarize][end]=>time:{el_time}")
    myutils.log_message(f"-" * 50)
